...
いくつかの報酬関数や動作モード、学習時間を変更して確認してみたが以下の点がわかった。結果
  ...
  * 最大speedと`完走率`はかなり関係がある。
    * 最大speedが大きい ⇒ `完走率`が低下する。
    * 最大speedが小さい ⇒ `完走率`が向上する。

| 順位 | 実験番号 | 完走率[%] |　時間 [sec]| 最大speed設定 [m/sec]　|
---- | ---- | ---- | ---- |---- |
|1 | 実験3(ジグザグ抑止) | 100 | 12.89 | 12  |
|2 | 実験9(ステアリング意識) | 100 | 33.968 | 3 |
|3 | 実験9(ステアリング意識) | 72 | 34.643 | 3 |

* まとめ  
報酬関数にも左右されると思うが、上記より、最大speedを低下させると完走率が確かに向上するようだ。
結構やってみた完走では、最大speedと完走率のトレードオフのような気がする。
